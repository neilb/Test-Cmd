...
use Test;
BEGIN { $| = 1; plan tests => 29, onfail => sub { $? = 1 if $ENV{AEGIS_TEST} } }
END {print "not ok 1\n" unless $loaded;}
use Test::Cmd;
$loaded = 1;
ok(1);
...
ok($string eq "NO RESULT for test of run at line 4 of -.\n");
...
ok($string eq "NO RESULT for test of run [xyzzy] at line 4 of -.\n");
...
ok($string eq "Printed on no result.\nNO RESULT for test of run at line 4 of -.\n");

#
$ret = open(PERL, "|$^X -w @I_FLAGS >perl.stdout.4 2>perl.stderr.4");
ok($ret);
...
$string = contents("perl.stdout.4");
ok($string eq "");
$string = contents("perl.stderr.4");
ok($string eq "NO RESULT for test of run at line 5 of - (main::test_it)\n\tfrom line 8 of -.\n");
